...
import com.example.androidasmt.databinding.FragmentStatusBinding
...
import com.example.androidasmt.MainActivity
...
class StatusFragment : Fragment() {
    ...
    private val vm: StatusFragmentModel by viewModels()
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View {
        ...
        binding.txtProceed.setOnClickListener {
            nav.navigate(
                R.id.action_statusFragment_to_qrCodeScannerFragment,
                bundleOf("id" to vm.id)
            )
        }
        ...
        vm.completed.observe(viewLifecycleOwner) { c ->
            ...
            // if c is not null, means that it is not progressing ------------
            // then remove the qr code link and cancel link
            if (c != null) {
    ...
    private fun cancelAppointment() {
        ...
        lifecycleScope.launch {
            ...
            val response = withContext(Dispatchers.IO) {
                VaccineApi.retrofitService.cancelBooking(vm.id)
            }
